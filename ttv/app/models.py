...
events_organisors = db.Table('events_organisors',
        db.Column('event_id', db.Integer(), db.ForeignKey('events.id')),
        db.Column('user_id', db.Integer(), db.ForeignKey('user.id')))
...
class User(db.Model, UserMixin):
    ...
    connections = db.relationship('Connection',
            backref=db.backref('users', lazy='joined'), cascade="all")
    def __str__(self):
...
class Events(db.Model):
    ...
    description=db.Column(db.String(255))
    organisor = db.relationship('User', secondary=events_organisors,
            backref=db.backref('organises', lazy='dynamic'))   
    def __str__(self):
        return "id :%s , event_id :%s , name :%s , view_name :%s , time :%s , category :%s , description:%s,organisor:%s"%(self.id,self.event_id, self.name, self.view_name, self.time, self.category, self.description,self.organisor)
    def __repr__(self):
...
class Schedule(db.Model):
    ...
    time = db.Column(db.DateTime())
    def __str__(self):
        return"<day:%s,venue:%s,time:%s>"%(self.day,self.venue,self.time)
    def __repr__(self):
        return"<day:%s,venue:%s,time:%s>"%(self.day,self.venue,self.time)    