...
def make_organisor(eventName,email):
	user = get_user(email)
	event = Events.query.filter_by(view_name=eventName).first()
	check2 = contains(event.organisor, lambda x: x.email == email)
	if check2 is None:
		user_datastore.add_role_to_user(user,"event_organisor")
		event.organisor.append(user)
		user_datastore.put(event)
		db.session.commit()
		return True
	return False
...
def create_event(name,view_name):
	event = Events(event_id="",name=name,view_name=view_name,category="")
	check = Events.query.filter_by(view_name=view_name).first()
	if check is None:
		user_datastore.put(event)
		db.session.commit()
		return True
...
def get_event(event):
	res = Events.query.filter_by(view_name=event).first()
	return res
...
def get_event_users(eventName):
	even = get_event(eventName)
	return even.users.all()
